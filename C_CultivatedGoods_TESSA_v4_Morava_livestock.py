...
country2 = "Czech Republic" # "Slovenia" # "Serbia"
countryCode = "CZ" #"SI" # "RS"

cultivated_good_type = "_livestock" # choose from "_agriculture" , "_fish", "_honey", "_livestock"
...
mostImportantLivestock_list = ["Cattle","Horses"] # Morava 1
# mostImportantLivestock_list = ["Beehives"] # Morava 2
...
heads_livestock_pilotArea_per_yr = [mostImportantLivestock_list,numpy.repeat(None,len(mostImportantLivestock_list,))] # Krka 1, Morava 1
...
LivestockProducts_list =  ["Meat, cattle","Milk, whole fresh cow", "Meat, horse"] # Morava 1
# LivestockProducts_list =  ["Honey, natural"] # Morava 2
...
# mostImportantFish_list = ["Sterlet sturgeon - Acipenser ruthenus","Sea trout - Salmo trutta","Huchen - Hucho hucho","Peled - Coregonus peled","Grayling - Thymallus thymallus","Northern pike - Esox lucius","Roach - Rutilus rutilus","Orfe (=Ide) - Leuciscus idus","Rudd - Scardinius erythrophthalmus","Grass carp (=White amur) - Ctenopharyngodon idellus","Asp - Aspius aspius","Tench - Tinca tinca","Common nase - Chondrostoma nasus","Gobio gobio","Barbel - Barbus barbus","Bleak - Alburnus alburnus","Freshwater bream - Abramis brama","Vimba bream - Vimba vimba","Goldfish - Carassius auratus","Crucian carp - Carassius carassius","Common carp - Cyprinus carpio","Silver carp - Hypophthalmichthys molitrix","Wels(=Som) Catfish - Silurus glanis","European eel - Anguilla anguilla","Burbot - Lota lota","European perch - Perca fluviatilis","Pike-perch - Stizostedion lucioperca"] # Morava
...
if mostImportantFish_list is not None:
    ...
    for feat in features_layer:
        ...
        if attributes_table[idx_country] == country2:
    ...
    GrossPrices_fish_sum_EUR_per_year = 0.0
    for fishType in mostImportantFish_list:
        ...
        for f in Aquaculture_products_layer.getFeatures():
            if f[idx_ESS] == 'fish' : 
                ...
                f[idx_TON] = float(production_tmp)
                # print("production in tonnes of "+fishType+": " + str(f[idx_TON]))
                # prices of livestock products
                prices_tmp = fishValue_pilotArea_ton_year*float(f[idx_PERC])
                f[idx_EUR] = float(prices_tmp)
                # print("prices of "+fishType+": "  + str(prices_tmp))
                # update the feature
                Aquaculture_products_layer.updateFeature( f )
                GrossPrices_fish_sum_EUR_per_year = numpy.nansum([GrossPrices_fish_sum_EUR_per_year, prices_tmp])
            else:
                f[idx_TON] = float('nan')
                # print("production in tonnes of "+fishType+": " + str(f[idx_TON]))
                f[idx_EUR] = float('nan')
                Aquaculture_products_layer.updateFeature( f )
                # print("prices of "+fishType+": "  + str(f[idx_EUR]))
        # commit
        Aquaculture_products_layer.commitChanges()
    ...
    file_to_write = open(output_txt_file_path, 'w')
    file_to_write.write(str("GrossPrices_fish_sum_EUR_per_year: " + str(GrossPrices_fish_sum_EUR_per_year)+ " EUR/year"))
    # close 
    file_to_write.close()
