...
# -- IMPORT LIBRARY  --
# -------------------------------------------------------------------------------------------------------------
sys.path.insert(1, (tool_path +'Library_v2'))
...
mostImportantFish_list = None # ['unknown']
cultivated_good_type = ""
...
mostImportantCrops_list = None #["maize"] # ["barley","maize","wheat"]
#["apple",
#"barley",
#"bean",
#"greenbean",
...
mostImportantLivestock_list = [ "Sheep","Beehives"]
...
if mostImportantCrops_list is not None:
    ...
    for index in range(len(harvestedArea_raster_list)):
       print(str(index+1) + " of " + str(len(harvestedArea_raster_list)))
    ...
    for cropType in mostImportantCrops_list:
       ...
       for f in Agriculture_products_layer.getFeatures():
           if f[idx_ESS] == 'agricultural product' : 
               ...
               Agriculture_products_layer.updateFeature( f )
               # check manually
               print("costs: "+ str(f[idx_cost] ) + "; net: "+ str(f[idx_net] ))
           else:
               f[idx_pp] = float('nan')
    ...
    QgsVectorFileWriter.writeAsVectorFormat(Agriculture_products_layer, str(tool_path + "/OutputData_ES/C_cultivated_goods/C_cultivated_output04_agriculture.shp"),"UTF-8", Agriculture_products_layer.crs(), "ESRI Shapefile")
...
if mostImportantFish_list is not None:
    ...
    QgsVectorFileWriter.writeAsVectorFormat(Aquaculture_products_layer, str(tool_path + "/OutputData_ES/C_cultivated_goods/C_cultivated_output06_fish.shp"),"UTF-8", Aquaculture_products_layer.crs(), "ESRI Shapefile")
